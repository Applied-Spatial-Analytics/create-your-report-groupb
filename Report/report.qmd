...
In order to get a clear answer to this question, this is divided into two sub-questions;
...
To evaluate the relationship between urban green infrastructure allocation and socio-economic vulnerability, this study bridges urban climate and social equity by quantifying the land surface temperature (LST) distribution alongside several metrics of spatial justice. The methodology is structured as follows: defining the metrics for measuring LST and spatial justice, explaining the data sources used for the analysis, and integrating these dimensions through a Multi-Criteria Decision Analysis (MCDA) framework.

As @bartesaghi-koc-2020 describe, the UHI effect is a phenomenon where urban areas experience higher temperatures than their rural surroundings. This is the effect of the built environment, which absorbs and retains heat, leading to elevated temperatures in cities. In addition, lack of vegetation and green spaces in cities contribute to this effect due to reduced evapotranspiration and shading. Therefore the presence of green infrastructure is a key factor in mitigating UHI [@bartesaghi-koc-2020].
...
- LST and how we obtained that - Viktor explain your workflow here!
...
For both Rotterdam and Guangzhou, the analysis relies on a combination of spatial and socio-economic datasets. For Rotterdam, data was primarily sourced from the *Leefbaarometer 2024* and *CBS Buurtgegevens*, which provide comprehensive neighborhood-level information on physical livability, social cohesion, housing conditions, and demographic characteristics. These datasets are complemented by high-resolution Land Surface Temperature (LST) data derived from satellite imagery, enabling precise mapping of urban heat distribution.

For Guangzhou, data acquisition posed a greater challenge due to limited availability of granular socio-economic information. Consequently, the analysis utilized OpenStreetMap (OSM) data to extract residential land use and green space locations, supplemented by pedestrian network data to assess accessibility. LST data for Guangzhou was obtained from remote sensing sources, ensuring comparability with the Rotterdam dataset. For population data for Guangzhou, which is similar to the data provided by CBS, we used open data from "WorldPop.org". However, as it is a global dataset, it may not be as accurate as for example the CBS data for Rotterdam. The integration of these diverse datasets allows for a robust cross-city comparison of spatial justice and urban climate vulnerability.

As we wanted to compare the two cities, we had to make sure that the data was comparable. Therefore, we used the same metrics for both cities. In order to make the analysis for both cities comparable, we had to make sure that for both cities we used the same amount of data. As there is much more data available for Rotterdam, we had to make sure that we only used the data that was also available for Guangzhou. This means that we had to leave out some of the data for Rotterdam, which is a limitation of this study.
...
- Rotterdam: neighborhood data from CBS - including most spatial justice metrics, Leefbaarometer 2024, buildings from the BAG (including function), road network and green areas from OSM

- Guangzhou: building/road/green area data from OSM, inhabitant data from ?

**Explanation of the code:**

- Spatial Justice Index; R code available in the "code" folder of the repository, it uses data from the "data" folder of the repository. Is used to combine different Spatial justice indicators into one index

  - we have to make separate files for the complete SJI code and the simplified SJI of rotterdam so they can be readily used

- Network analysis; R code available in the "code" folder. Takes multiple datasets as input (available in the data folder). is used to compute the distance from houses to green areas. And it simultaneously plots it, aggregated to hexagons, over the UHI

- I (Anna) think the other code files were not really used but not completely sure
...
![Distance to green vs LST in Rotterdam](maps/results_lst_distance_to_green/climate_vul_rotterdam.png){#fig-rot-green-lst fig-pos="H" fig-align="center"}
...
**TO DO: put in some code from the R file here**
...
![Spatial Justice Index per neighborhood in Rotterdam](maps/SJI_Rotterdam_Full.png){#fig-sji-rot-full fig-pos="H" fig-align="center"}

### Simplified Spatial Justice Index

**TO DO: explain what metrics were used in this map**

![Spatial Justice Index per neighborhood in Rotterdam comparable to Guangzhou](maps/RotterdamSJI.png){#fig-rot-sji-simple fig-pos="H" fig-align="center"}
...
In order to retrieve cadastral information of Guangzhou, the relevant OpenStreetMap (OSM) polygons for general residential land use were first queried using QuickOSM and visualised in QGIS. To avoid relying on overly large, generalized areas, these polygons were subdivided into a standard 100-meter vector grid, effectively generating smaller raster cells that simulate individual building footprints. In addition, green spaces were identified by extracting multiple vegetation categories, such as forests, parks, and grasslands from again using QuickOSM.
...
The land surface temperature (LST) data for Guangzhou was obtained from LANSAT satellite imagery for the period of **DO you know this? @Viktor** .
...
![Distance to green vs LST in Guangzhou](maps/results_lst_distance_to_green/climate_vul_guangzhou.png){#fig-green-lst-guangzhou fig-pos="H" fig-align="center"}
